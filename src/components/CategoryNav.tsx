...
export const CategoryNav: React.FC<CategoryNavProps> = ({
  ...
  return (
    <div className="sticky top-16 sm:top-20 z-30 bg-[#071f19]/95 backdrop-blur-md border-b border-[#123e32] py-3 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Quick Type Filter Chips */}
        <div className="flex items-center space-x-2 pb-2.5 mb-2.5 overflow-x-auto no-scrollbar border-b border-[#10382d]">
          <button
            onClick={() => setActiveFilter('all')}
            className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
              activeFilter === 'all'
                ? 'bg-[#e5a93c] text-black font-semibold shadow'
                : 'bg-[#0c2a22] text-gray-300 hover:text-white border border-[#164437]'
            }`}
          >
            Все позиции
          </button>
          ...
          <button
            ...
            className={`flex items-center px-4 py-2 rounded-xl text-xs sm:text-sm font-medium whitespace-nowrap transition-all border ${
              activeCategory === 'all_categories'
                ...
                : 'bg-[#0a261f] text-gray-300 border-[#123e32] hover:border-[#1a5545]'
            }`}
          >
            Все категории
          </button>
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium whitespace-nowrap transition-all border ${
                  isActive
                    ...
                    : 'bg-[#0a261f] text-gray-300 border-[#123e32] hover:border-[#1a5545] hover:text-white'
                }`}
              >
                <IconComponent className={`w-4 h-4 ${isActive ? 'text-black' : 'text-[#e5a93c]'}`} />
                <span>{cat.title}</span>
                <span className={`ml-1 text-[10px] px-1.5 py-0.2 rounded-full ${
                  isActive ? 'bg-black/20 text-black' : 'bg-[#0e3027] text-gray-300'
                }`}>
                  {cat.items.length}
                </span>
              </button>
